...
class TomTomProvider implements GeocodingProvider {
  ...
  async reverseGeocode(lat: number, lng: number): Promise<string | null> {
    ...
    try {
      ...
      const response = await withRetry(
        () => fetch(`https://api.tomtom.com/search/2/reverseGeocode/${lat},${lng}.json?${queryParams.toString()}`),
        { maxAttempts: 2, baseDelay: 1000 },
      )
      ...
      logger.error(
        "TomTom reverse geocoding failed",
        {
          component: "geocoding",
          action: "reverse_geocode_error",
...
class GeocodingManager {
  ...
  constructor() {
    this.providers = [new MapboxProvider(), new TomTomProvider(), new FallbackProvider()]
  }
