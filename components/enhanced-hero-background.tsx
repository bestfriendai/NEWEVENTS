...
export default function EnhancedHeroBackground() {
  ...
  useEffect(() => {
    ...
    const backgroundParticles = createParticleSystem(scene, 3000, 100, [0.4, 0.1, 0.6], 0.2, 0.6) // Background layer
    const middleParticles = createParticleSystem(scene, 1500, 70, [0.6, 0.2, 0.8], 0.3, 0.8) // Middle layer
    const foregroundParticles = createParticleSystem(scene, 800, 50, [0.8, 0.3, 1.0], 0.4, 1.0) // Foreground layer

    particlesRef.current = [backgroundParticles, middleParticles, foregroundParticles]
    ...
    const animate = () => {
      ...
      particlesRef.current.forEach((particles, index) => {
        if (particles) {
          ...
          particles.rotation.y += 0.0002 + (particles.userData.speed || 0)

          // Subtle pulsing effect
          const scale = 1 + Math.sin(timeRef.current * 2) * 0.03 * (particles.userData.pulseIntensity || 1)
          particles.scale.set(scale, scale, scale)

          // Responsive to mouse movement
          particles.rotation.x += mouseRef.current.y * 0.0002 * (particles.userData.mouseResponsiveness || 1)
