...
export default function EventsMap({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const userPopup = new mapboxgl.Popup({ offset: 25, closeButton: false }).setHTML(`
  ...
  }, [userLocation, mapStyle, onError])

  // Create event markers
  const createEventMarker = useCallback(
    (event: EventDetailProps) => {
      if (!map.current || !event.coordinates) return null
      ...
      const categoryColor =
        categoryColors[event.category?.toLowerCase() as keyof typeof categoryColors] || "from-purple-500 to-pink-500"

      markerElement.innerHTML = `
        <div class="relative group cursor-pointer">
          <div class="w-8 h-8 bg-gradient-to-r ${categoryColor} rounded-full border-2 border-white shadow-lg transform transition-all duration-200 group-hover:scale-110 flex items-center justify-center">
          ...
          <div class="flex items-start space-x-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center justify-between mt-2">
                <span class="text-purple-400 font-semibold text-sm">${event.price}</span>
  ...
  }, [events, mapLoaded, createEventMarker, userLocation, onEventSelect])
