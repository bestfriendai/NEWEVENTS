...
import { safeEventsAPI, type SafeEvent } from "@/lib/api/safe-events-api"
import {
  safeLocationService,
  type SafeUserLocation,
  type SafeLocationError,
} from "@/lib/services/safe-location-service"
...
export default function SafeEventsExplorer() {
  // Location state
  const [userLocation, setUserLocation] = useState<SafeUserLocation | null>(null)
  ...
  const [filteredEvents, setFilteredEvents] = useState<SafeEvent[]>([])
  const [selectedEvent, setSelectedEvent] = useState<SafeEvent | null>(null)
  ...
  const handleGetCurrentLocation = useCallback(async () => {
    ...
    try {
      const location = await safeLocationService.getCurrentLocation()
      setUserLocation(location)
      setLocationInput(location.displayName)
      setShowLocationSelector(false)

      logger.info("Current location set", {
        component: "SafeEventsExplorer",
        action: "getCurrentLocation",
        metadata: { location: location.displayName },
      })
    } catch (error) {
      const locationError = error as SafeLocationError
  ...
  const debouncedLocationSearch = useCallback(
    debounce(async (query: string) => {
      ...
      try {
        const location = await safeLocationService.searchLocation(query)
        setUserLocation(location)
        setLocationInput(location.displayName)
        setShowLocationSelector(false)
  ...
  // Search for events with better error handling
  const searchEvents = useCallback(async () => {
    ...
    try {
      ...
      const foundEvents = await safeEventsAPI.searchEvents(searchParams)
            ...
            <CardContent className="p-8">
              ...
              <div className="space-y-6">
                ...
                <Button
                  ...
                  disabled={isLocationLoading || !isOnline}
                  size="lg"
