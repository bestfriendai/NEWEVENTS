...
export function usePerformanceMonitor(): UsePerformanceMonitorResult {
  ...
  const stopMonitoring = () => {
    ...
    setMetrics(finalMetrics as PerformanceMetrics)
  ...
  const trackApiCall = useCallback((endpoint: string, startTime: number, endTime: number, success: boolean) => {
    ...
    const interval = setInterval(() => {
      ...
      setMetrics(
        (prev) =>
          ({
            ...prev,
            responseTime: performance.now() - pageLoadTime,
            memoryUsage: Math.round(memory * 100) / 100,
            renderTime: performance.now() - pageLoadTime,
  ...
  }, [trackUserInteraction, trackScrollDepth, getWebVitals, getMemoryUsage, getNetworkInfo])
