...
export function useRealtimeEvents(options: UseRealtimeEventsOptions = {}) {
  const { enabled = true, userLocation, radius = 25, autoReconnect = true } = options
  ...
  const initializeSupabase = useCallback(() => {
    if (!supabaseRef.current) {
      supabaseRef.current = createClient(env.NEXT_PUBLIC_SUPABASE_URL!, env.NEXT_PUBLIC_SUPABASE_ANON_KEY!, {
        realtime: {
          params: {
            eventsPerSecond: 10,
          },
    ...
    (payload: RealtimeEvent) => {
      ...
      try {
        if (type === "INSERT" && record) {
          ...
          // Update live events
          setLiveEvents((prev) => prev.map((event) => (event.id === record.id ? updatedEvent : event)))
